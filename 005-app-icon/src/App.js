...
const Icon = React.createClass({
  ...
  fetchIP: function() {
    fetch('//freegeoip.net/json/')
      ...
      .then(data => {
        let city = data.city || defaultCity;
  ...
  render: function() {
    return (
      <div className="Icon" data-hour={this.state.time}>
        ...
        <WeatherIcon
          src={this.state.icon}
          weatherCode={this.state.weather_code}
          timeOfDay={this.state.time}
        />
        <div className="Information">
          <div className="Location">{this.state.location}</div>
          <div className="Temperature">{this.state.temp} ° F</div>
        </div>
      </div>
    );
  }
...
const WeatherIcon = React.createClass({
  render: function() {
    let timeOfDay = this.props.timeOfDay > 7 && this.props.timeOfDay < 18
      ? 'day'
      : 'night';
